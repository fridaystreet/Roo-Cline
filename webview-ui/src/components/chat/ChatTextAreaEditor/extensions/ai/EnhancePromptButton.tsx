...
export const EnhancePromptButton = ({ editor }: { editor: Editor | null | undefined }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center" }}>
      {editor?.storage?.ai?.isEnhancing && <span style={{ marginRight: 10, color: "var(--vscode-input-foreground)", opacity: 0.5 }}>Enhancing prompt...</span>}
      <span
        role="button"
        ...
        onClick={aiCommand}
        style={{ fontSize: 16.5, color: editor?.storage?.ai?.isEnhancing ? 'yellow' : undefined }}
      />
    </div>
  )
}
